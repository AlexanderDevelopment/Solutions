...
{
    private readonly System.Random _sysRnd = new();
    private readonly Stack<double> _normalValues = new();
    System.Security.Cryptography.RNGCryptoServiceProvider crypto_rnd = new();
    ...
    public float Range(float min, float max) 
        => min + (float)GetRandomDouble() * (max - min);
    ...
    {
        if(_normalValues.Count > 0)
            return _normalValues.Pop();
        double u = 0;
        double v = 0;
        double s = 0;
        do
    ...
    public double NormalDistribution01() 
        => Clamp01(NormalDistribution() / 6.0 + 0.5);
    ...
    public double NormalDistribution11() 
        => Clamp(NormalDistribution() / 3.0, -1, 1);
